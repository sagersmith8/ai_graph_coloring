...
def order_values(problem, variable, assignment, num_values):
    ...
    unusable_colors = set(assignment.get(node) for node in problem[variable])
    ...
    return [count[1] for count in (
        sorted((count[index], index) for index in xrange(num_values) if index not in unusable_colors)
    )]
